        ...
        with tf.variable_scope(tf.get_variable_scope(), initializer=self.initializer,
            ...
            if self.layer_norm is None and self.hparams.norm in BaseRankingModel.NORM_FUNC_DIC:
                ...
                for j in range(len(output_sizes)):
                    self.layer_norm.append(nn.LayerNorm(output_data.size()[1:]))
            ...
            for j in range(len(output_sizes)-1):
                if self.layer_norm is not None:
                    if self.hparams.norm == "layer":
                        ...
                        output_data = self.layer_norm[j](
                            output_data)
                linear_layer = nn.Linear(output_sizes[j], output_sizes[j+1])
                linear_layer_weight = linear_layer.weight
                ...
                output_data = torch.matmul(output_data, expand_W) + expand_b
